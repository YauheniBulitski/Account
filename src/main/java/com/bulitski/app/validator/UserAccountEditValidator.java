...
@Component
@NoArgsConstructor
public class UserAccountEditValidator implements CustomValidator<UserAccountDto> {

    private UserServiceImpl userService;

    @Autowired
    public UserAccountEditValidator(UserServiceImpl userService) {
    ...
    public void check(UserAccountDto userAccountDto, Errors errors) {
        if (userAccountDto.getUserName().length() < NumberConstant.THREE || userAccountDto.getUserName().length() > NumberConstant.SIXTEEN) {
            ...
            UserAccount userAccount = userService.findByUserName(userAccountDto.getUserName());
            if (userAccount != null && userAccount.getId() != userAccountDto.getId()) {
